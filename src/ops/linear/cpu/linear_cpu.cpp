...
void linear_(std::byte *out, const std::byte *in, const std::byte *weight, const std::byte *bias,
    ...
    // Perform matrix multiplication: out = in * weight^T + bias
    // This should match PyTorch's torch.nn.functional.linear(x, weight, bias)
    // which computes: x @ weight.T + bias
    for (size_t b = 0; b < batch_size; b++) {
        for (size_t o = 0; o < out_features; o++) {
            // Initialize with bias if provided, otherwise zero
            ...
            for (size_t i = 0; i < in_features; i++) {
                ...
                T weight_val = weight_ptr[o * in_features + i]; // weight[o, i] = weight.T[i, o]
